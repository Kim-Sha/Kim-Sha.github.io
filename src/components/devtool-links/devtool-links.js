    ...
    <div className="pl-3" id="contact">
      {devtool.map(({ name, url }, i) => {
        return (
          <a
            aria-label={name}
            className={`inline-flex w-10 h-10 md:w-12 md:h-12 justify-center items-center rounded-full -ml-3 bg-back-3 text-front-3 hover:bg-front-3 hover:text-back-3`}
            href={url}
            key={name}
