    ...
    <header className="p-4 bg-none sticky lg:static top-0 z-40 space-y-0">
      <div className={`${toggle ? 'rounded-b-none' : null} rounded-lg bg-front flex justify-between p-4 lg:px-8`}>
        ...
        <div className="hover:text-primary">
          <Link to='/'>
            <span className="inline-flex w-8 h-8 lg:w-14 lg:h-14 lg:mt-4 font-header font-bold text-xs lg:text-xl justify-center items-center text-center text-back lg:text-front border-2 border-solid border-back lg:border-front rounded-full">
              {initials}
            </span>
          </Link>
        </div>
      ...
      <div className={`${toggle ? 'block' : 'hidden'} rounded-t-none px-2 pb-4 lg:hidden bg-front rounded-lg`}>
